-- 202605220005_layering_entries.sql
-- User-recorded fragrance layering combos.
-- fragrance_a_id < fragrance_b_id enforces canonical ordering so
-- (A+B) and (B+A) are the same entry.
...
create index layering_entries_frag_a_idx on layering_entries (fragrance_a_id);
create index layering_entries_frag_b_idx on layering_entries (fragrance_b_id);
