/**
 * Scrape Memo Paris fragrances via Shopify JSON API.
 * Source: https://www.memoparis.com/collections/all/products.json
 */
...
const OUT_PATH = path.join(DATA_DIR, 'frag-memo-paris-shopify-raw.json');
const BASE = 'https://www.memoparis.com';
...
async function main() {
  ...
  const fragrances = allProducts.filter(p => {
    ...
    if (t.includes('voucher') || t.includes('gift card') || t.includes('discovery set')) return false;
    if (t.includes('candle') || t.includes('soap') || t.includes('lotion')) return false;
    if (tags.some(tg => tg.includes('notsync') || tg.includes('hide_from_search'))) return false;
    // Must have variants with ml sizes or be an EDP/EDT
    const h = p.handle.toLowerCase();
    if (h.includes('eau-de-parfum') || h.includes('extrait') || h.includes('parfum')) return true;
    const hasSize = p.variants.some(v => parseSizeMl(v.title) !== null);
    return hasSize;
  });
  ...
  for (const [i, product] of fragrances.entries()) {
    ...
    const prices = product.variants
      .map(v => ({ retailer: 'memoparis-direct', size_ml: parseSizeMl(v.title) ?? 0,
    ...
    out.push({
      brand: 'Memo Paris', name: product.title, release_year: null, concentration,
      ...
      source: 'memoparis', source_url: `${BASE}/products/${product.handle}`, sources: ['memoparis'],
    });
