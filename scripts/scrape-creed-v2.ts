/**
 * Scrape Creed fragrances via Shopify JSON API.
 * Source: https://www.creedboutique.com/collections/fragrances/products.json
 */
...
const OUT_PATH = path.join(DATA_DIR, 'frag-creed-shopify-raw.json');
const BASE = 'https://www.creedboutique.com';
...
function parseGender(tags: string[], title: string): 'feminine' | 'masculine' | 'unisex' | null {
  ...
  if (t.includes('women') || t.includes('feminine') || t.includes('for-her') || t.includes('for her')) return 'feminine';
  if (t.includes('men') || t.includes('masculine') || t.includes('for-him') || t.includes('for him')) return 'masculine';
  return 'unisex';
}
...
async function fetchPage(page: number): Promise<ShopifyProduct[]> {
  const collections = ['womens-fragrances', 'mens-fragrances', 'unisex-fragrances'];
  const products: ShopifyProduct[] = [];
  for (const col of collections) {
    const url = `${BASE}/collections/${col}/products.json?limit=250&page=${page}`;
...
async function main() {
  ...
  const fragrances = allProducts.filter(p => {
    ...
    return !t.includes('set') && !t.includes('candle') && !t.includes('lotion')
      && !t.includes('gift card') && !t.includes('sample') && !t.includes('engraving')
      && !t.includes('coffret') && !t.includes('travel spray');
  });
  ...
  for (const [i, product] of fragrances.entries()) {
    ...
    const prices = product.variants
      .map(v => ({ retailer: 'creed-direct', size_ml: parseSizeMl(v.title) ?? 0,
    ...
    out.push({
      brand: 'Creed', name: product.title, release_year: null, concentration,
      fragrance_family: null, gender, top_notes: notes.top, heart_notes: notes.heart,
      ...
      source: 'creed', source_url: `${BASE}/products/${product.handle}`, sources: ['creed'],
    });
