...
describe('routeDnaCta — trait-routed monetization CTA (M6)', () => {
  ...
  it('defaults to the Original when no routing trait scored (no sample CTA exists)', () => {
    expect(routeDnaCta(values({ collector: 0.8, complimentSeeking: 0.6, expressive: 0.4 }))).toEqual(DEFAULT_DNA_CTA);
    expect(routeDnaCta(values({})).kind).toBe('original');
    // adventurous is a taste trait now — it routes nowhere and falls to the default.
    expect(routeDnaCta(values({ adventurous: 0.9 })).kind).toBe('original');
  });
  ...
  it('every CTA carries a non-empty label and sub', () => {
    for (const v of [{ valueHunter: 0.9 }, { luxury: 0.9 }, {}]) {
